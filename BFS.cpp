...
BFS::BFS( const Network net, const int seed ){
    Crawl(net.getNodes(), net.getV(), net.getE(), seed);
}

void BFS::Crawl(vector<Node> nds, const int nv, const int ne, const int s){
    ...
    memoryInUse.push_back((int)(que.size()));
    
    const int nvs = countVSeen(nodes);
    Vseen.push_back(nvs);
//    Vseen.push_back((int)links.size()+1);
//    const int nes = countCurrentESeen(nodes, links);
    ...
    while( Vcrawled.size()<V ){
        ...
        if( que.empty() ){
            ...
            que.push(nodes.at(dstIndex).getNodeID());
...
int BFS::findUncrawledNode(vector<Node> nds){
    for( ; nidx<V; nidx++ ){
